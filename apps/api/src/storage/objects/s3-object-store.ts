...
export const makeS3ObjectStore = (
  ...
  return {
    ...
    ...multipartOperations(client, location.bucket, options.allowedOrigins ?? []),
    ...
    head: async (key, versionId, signal) => {
      const result = await client
        .send(
          new HeadObjectCommand({ Bucket: location.bucket, Key: key, VersionId: versionId }),
          objectRequestOptions(signal),
        )
        .catch((error: unknown) => (missingObject(error) ? null : providerFailure(error)));
      return result === null ? null : objectFacts(result);
    },
    read: async (key, range, versionId, signal) => {
      ...
      return { ...objectFacts(result), body: result.Body };
    },
    remove: async (key, versionId, signal) => {
      await client
        .send(
          new DeleteObjectCommand({ Bucket: location.bucket, Key: key, VersionId: versionId }),
          objectRequestOptions(signal),
        )
        .catch(providerFailure);
    },
